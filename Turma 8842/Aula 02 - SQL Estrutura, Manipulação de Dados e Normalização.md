...
```bash
# Terminal
cp /vagrant/files/Foto.png /var/lib/mysql-files/
```

Carrega o conteúdo da imagem no campo `foto` do cientista com ID 1:
```sql
-- Inserção de imagem
UPDATE cientistas SET foto = LOAD_FILE('/var/lib/mysql-files/Foto.png') WHERE id = 1;
```
...
## 5. Criação de tabelas auxiliares e relacionamento
...
## 7. Exportação de dados para arquivo
